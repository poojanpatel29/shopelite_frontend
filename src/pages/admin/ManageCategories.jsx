...
export default function ManageCategories() {
  ...
  const handleAdd = () => {
    const newCat = { id: Date.now(), name: form.name, slug: form.name.toLowerCase().replace(/\s+/g, '-'), icon: form.icon, productCount: 0 }
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Add Category">
        <div className="space-y-4">
          <Input label="Category Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Furniture" />
          <Input label="Icon (emoji)" value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} placeholder="📦" />
          <Button className="w-full" onClick={handleAdd} disabled={!form.name}>Add Category</Button>
        </div>
      </Modal>
    </div>
  )
