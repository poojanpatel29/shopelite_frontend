...
import { selectAllProducts, deleteProduct, setProducts, setTotal } from '../../redux/slices/productsSlice'
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {filtered.map((p) => (
                <tr key={p.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {p.discount > 0 && <Badge variant="danger" className="text-[10px]">-{p.discount}%</Badge>}
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <Badge variant={p.stock > 20 ? 'success' : p.stock > 0 ? 'warning' : 'danger'}>{p.stock}</Badge>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      ...
                      <Button size="sm" variant="danger" onClick={() => handleDelete(p.id, p.name)}>Delete</Button>
                    </div>
                  </td>
                </tr>
              ))}
