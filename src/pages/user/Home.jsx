...
import { setProducts, selectAllProducts, selectFeaturedProducts } from '../../redux/slices/productsSlice'
...
export default function Home() {
  ...
  const newest = useMemo(() => [...all].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)).slice(0, 4), [all])
  const deals = useMemo(() => all.filter((p) => p.discount > 0).slice(0, 4), [all])

  useEffect(() => {
    // Only fetch if store is empty (avoids refetch on every visit)
    if (all.length === 0) {
      productsApi.getAll({ per_page: 20 }).then((data) => {
  ...
  return (
    <div>
      ...
      <section className="relative bg-gradient-to-br from-gray-900 via-primary-900 to-gray-900 text-white overflow-hidden">
        <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(circle at 20% 50%, #0ea5e9 0%, transparent 50%), radial-gradient(circle at 80% 50%, #7c3aed 0%, transparent 50%)' }} />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 text-center md:text-left">
            ...
            <h1 className="text-5xl md:text-6xl font-display font-bold leading-tight mb-6">
              Shop the <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary-400 to-cyan-400">Future</span><br />Today
            </h1>
            <p className="text-gray-300 text-lg mb-8 max-w-md">
              Discover thousands of premium products. Fast delivery across India, easy returns, and unbeatable prices.
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
              <Link to="/products" className="btn-primary text-lg px-8 py-3 rounded-xl">Shop Now →</Link>
              <Link to="/products?filter=deals" className="btn-secondary text-lg px-8 py-3 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white">View Deals</Link>
            </div>
          </div>
          <div className="flex-1 flex justify-center">
            <div className="relative w-80 h-80">
              <div className="absolute inset-0 bg-gradient-to-br from-primary-500/30 to-purple-500/30 rounded-3xl blur-3xl" />
              <img src="https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?w=600" alt="Shopping" className="relative rounded-3xl object-cover w-full h-full shadow-2xl" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-primary-600 text-white py-6">
        <div className="max-w-7xl mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[['1L+', 'Products'], ['50L+', 'Happy Customers'], ['100%', 'Secure Checkout'], ['24/7', 'Customer Support']].map(([val, label]) => (
            <div key={label}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
          {CATEGORIES.map((cat) => (
            <Link key={cat.id} to={`/products?category=${cat.slug}`}
              ...
              <h3 className="font-medium text-gray-900 dark:text-white text-sm group-hover:text-primary-600 transition-colors">{cat.name}</h3>
      ...
      <section className="bg-gradient-to-r from-accent to-red-500 text-white py-12 mb-12">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <p className="text-orange-100 mb-6">Up to 50% off — Diwali, Holi & more. Don't miss out!</p>
          <Link to="/products?filter=deals" className="bg-white text-accent hover:bg-gray-50 font-semibold px-8 py-3 rounded-xl transition-colors inline-block">Grab the Deals →</Link>
        </div>
      </section>
