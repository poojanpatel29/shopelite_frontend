...
export default function AddressBook() {
  ...
  const [form, setForm] = useState({ label: 'Home', fullName: '', street: '', city: '', state: '', zip: '', country: 'USA', isDefault: false })
          ...
          {addresses.map((addr) => (
            <div key={addr.id} className="card p-6">
              <div className="flex items-start justify-between mb-3">
                ...
                <button onClick={() => handleDelete(addr.id)} className="text-gray-400 hover:text-red-500 transition-colors text-sm">Delete</button>
              </div>
          ...
          <div className="flex gap-3">
            {['Home', 'Work', 'Other'].map((l) => (
              <button key={l} onClick={() => setForm({ ...form, label: l })}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${form.label === l ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'}`}>{l}</button>
            ))}
          </div>
