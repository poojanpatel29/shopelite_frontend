  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      dispatch(
        addToast({
          type: 'error',
          message: err.response?.data?.detail || 'Order failed. Please try again.',
        })
      );
    } finally {
      setLoading(false);
        ...
        {STEPS.map((s, i) => (
          <div key={s} className="flex items-center">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`text-sm font-medium ${i === step ? 'text-primary-600' : 'text-gray-500'}`}
              >
                {s}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div
                className={`h-0.5 w-16 mx-3 ${i < step ? 'bg-green-500' : 'bg-gray-200 dark:bg-gray-700'}`}
              />
            )}
          </div>
        ))}
            ...
            <div className="card p-6 space-y-4">
              ...
              {!showNewForm ? (
                ...
                <div className="border-2 border-primary-200 dark:border-primary-800 rounded-xl p-4 space-y-3">
                  ...
                  <Input
                    label="Street"
                    placeholder="12, MG Road, Banjara Hills"
                    {...f('street')}
                    required
                  />
                  <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setNewAddress((p) => ({ ...p, is_default: e.target.checked }))
                      }
                  ...
                  <div className="flex gap-3">
                    ...
                    {addresses.length > 0 && (
                      <Button onClick={() => setShowNewForm(false)} variant="secondary" size="sm">
                        Cancel
                      </Button>
                    )}
                  </div>
          ...
          {step === 1 && (
            <div className="card p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <Button variant="secondary" onClick={() => setStep(0)} size="lg" className="flex-1">
                  Back
                </Button>
              ...
              {items.map((item) => {
                ...
                return (
                  <div key={item.id} className="flex items-center gap-3">
                    ...
                    <Price
                      amount={price * item.quantity}
                      className="text-sm font-semibold text-gray-900 dark:text-white flex-shrink-0"
                    />
                  </div>
                );
              })}
