  ...
  if (!product)
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        <div className="w-20 h-20 bg-zinc-100 dark:bg-zinc-800 rounded-3xl flex items-center justify-center text-4xl mx-auto mb-6">
          😕
        </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <nav className="flex items-center gap-2 text-xs text-zinc-400 mb-8 font-medium">
        {[
          ...
          <span key={to} className="flex items-center gap-2">
            <Link to={to} className="hover:text-primary-600 capitalize transition-colors">
              {label}
            </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        ...
        <div className="space-y-5">
          ...
          <div>
            <p className="text-[11px] font-bold text-primary-500 uppercase tracking-widest mb-1">
              {product.category}
            </p>
            ...
            <div className="flex items-center gap-2">
              <div className="flex">
                {[1, 2, 3, 4, 5].map((s) => (
                  <svg
                    key={s}
                    className={`w-4 h-4 ${s <= Math.round(product.rating) ? 'text-amber-400' : 'text-zinc-200 dark:text-zinc-700'}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-1 bg-zinc-100 dark:bg-zinc-800 rounded-xl p-1">
              ...
              <span className="w-10 text-center font-bold text-zinc-900 dark:text-white text-sm">
                {quantity}
              </span>
              <button
          ...
          <div className="flex flex-wrap gap-2 pt-3 border-t border-zinc-100 dark:border-zinc-800">
            {[
              ...
              <span
                key={label}
                className="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800 px-3 py-1.5 rounded-full"
              >
                {icon} {label}
              </span>
            ))}
      ...
      <div className="card mb-16 overflow-hidden">
        ...
        <div className="p-6">
          {activeTab === 'description' && (
            <p className="text-zinc-600 dark:text-zinc-400 leading-relaxed text-sm max-w-2xl">
              {product.description}
            </p>
          )}
          {activeTab === 'specifications' && product.specifications && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-0 max-w-2xl">
              {Object.entries(product.specifications).map(([k, v], i) => (
                <div
                  key={k}
                  className={`flex gap-4 py-3 ${i % 2 === 0 ? 'border-b border-zinc-100 dark:border-zinc-800' : 'border-b border-zinc-100 dark:border-zinc-800'}`}
                >
                  <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider w-28 shrink-0 pt-0.5">
                    {k}
                  </span>
